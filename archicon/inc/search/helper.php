...
if ( ! function_exists( 'archicon_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function archicon_get_search_page_excerpt_length() {
		$length = apply_filters( 'archicon_filter_post_excerpt_length', 180 );

		return intval( $length );
	}
}

if ( ! function_exists( 'archicon_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function archicon_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'archicon_render_block_core_search';
		}

		return $atts;
	}

	add_filter( 'block_type_metadata_settings', 'archicon_override_search_block_templates' );
}

if ( ! function_exists( 'archicon_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function archicon_render_block_core_search( $attributes ) {
		static $instance_id = 0;

		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'archicon' ),
				'buttonText' => esc_html__( 'Search', 'archicon' ),
			)
		);

		$input_id            = sprintf( 'qodef-search-form-%s', ++$instance_id );
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'archicon' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = archicon_get_svg_icon( 'search' );
			}
