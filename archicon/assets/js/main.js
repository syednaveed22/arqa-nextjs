(function ($) {
	...
	$(document).on(
		'archicon_trigger_get_new_posts',
		function () {
			qodefSwiper.init();
			qodefMagnificPopup.init();
		}
			...
			return {
				on: {
					...
					init: function () {
						...
						qodef.body.trigger(
							'archicon_trigger_swiper_is_initialized',
							[$holder, options]
						);
	...
	var qodefMasonryLayout = {
				...
				this.holder.each(
					function () {
						var $masonry = $(this).find('.qodef-grid-inner'),
							$ultraSquareItem = $masonry.find('.qodef-item--ultra-huge-square');

						if ($ultraSquareItem.length) {
							$ultraSquareItem.css('max-width', $masonry.width());
						}
		...
		setFixedImageProportionSize: function ($holder, $item, size) {
			var padding = parseInt($item.css('paddingLeft'), 10),
				assortedItemMargin = parseInt($item.css('marginBottom'), 10),
				...
				$ultraSquareItem = $holder.find('.qodef-item--ultra-huge-square'),
				isMobileScreen = qodef.windowWidth <= 680;

			if ($item.hasClass('qodef-assorted-portfolio-gallery-item')) {
				$item.css('height', size * 0.7);
			} else {
				$item.css('height', size);
			}

			if ($landscapeItem.length) {
				if ($landscapeItem.hasClass('qodef-assorted-portfolio-gallery-item')) {
					$landscapeItem.css('height', Math.round(size * 0.7 / 2));
				} else {
			...
			if ($portraitItem.length) {
				if ($portraitItem.hasClass('qodef-assorted-portfolio-gallery-item')) {
					$portraitItem.css('height', Math.round(2 * size * 0.7 + assortedItemMargin));
				} else {
			...
			if (!isMobileScreen) {
				...
				if ($landscapeItem.length) {
					if ($landscapeItem.hasClass('qodef-assorted-portfolio-gallery-item')) {
						$landscapeItem.css('height', size * 0.7);
					} else {
				...
				if ($hugeSquareItem.length) {
					if ($hugeSquareItem.hasClass('qodef-assorted-portfolio-gallery-item')) {
						$hugeSquareItem.css('height', Math.round(2 * size * 0.7 + assortedItemMargin));
					} else {
				...
				if ($ultraSquareItem.length) {
					if ($ultraSquareItem.hasClass('qodef-assorted-portfolio-gallery-item')) {
						$ultraSquareItem.css('height', Math.round(2 * size * 0.7 + assortedItemMargin));
					} else {
						$ultraSquareItem.css('height', Math.round(2 * (size + padding)));
					}
				}
			}

			if ($ultraSquareItem.length) {
				$ultraSquareItem.css('max-width', $holder.width());
			}
		}
	};
...
(function ($) {
			...
			this.holder.push(
				{
					...
					options: {
						...
						dropdownCssClass       : 'qodef--ordering'
					}
				}
			);
			this.holder.push(
	...
	var qodefWooButtons = {
		init: function () {
			var $button = $('#qodef-woo-page .button:not(.qqvfw-shortcode), #qodef-woo-page .added_to_cart, .qodef-woo-product-content-inner .button:not(.qqvfw-shortcode), .qodef-woo-product-content-inner .added_to_cart, .woocommerce-widget-layered-nav-dropdown__submit, .price_slider_amount .button, .woocommerce-mini-cart__buttons .button, .return-to-shop .button, .woocommerce-form-coupon-toggle a, .place-order #place_order, .woocommerce-form-login__submit, .woocommerce-MyAccount-content .woocommerce-Button, .woocommerce-MyAccount-content .button, .wc-block-components-price-slider__button');

			if ($button.length) {
				$button.each(
					function () {
						var $thisButton = $(this),
							buttonText = $thisButton.hasClass('wc-block-components-price-slider__button') ? $thisButton.find('span:not(.screen-reader-text)').text() : $thisButton.text(),
							buttonIcon = '<svg class="qodef-svg--button-arrow" xmlns="http://www.w3.org/2000/svg" width="11.966" height="9.767" viewBox="0 0 11.966 9.767"><g transform="translate(-541.5 -761.786)"><line x2="11.077" transform="translate(541.5 766.628)"></line><path d="M579.5,677.139l4.53,4.53-4.53,4.53" transform="translate(-31.271 85)"></path></g></svg>',
